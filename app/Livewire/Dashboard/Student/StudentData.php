...
class StudentData extends Component
{
    ...
    {
        $query = Grade::with('stage:id,name')->where('is_active', true);
    ...
    public function getStudentQuery(): Builder
    {
        ...
        if ($this->search_week_id) {
            $gradeIds = [Week::find($this->search_week_id)?->semester?->grade_id];
        } elseif ($this->search_semester_id) {
            $gradeIds = [Semester::find($this->search_semester_id)?->grade_id];
        } elseif ($this->search_grade_id) {
            $gradeIds = [$this->search_grade_id];
        } elseif ($this->search_stage_id) {
            $gradeIds = Grade::where('stage_id', $this->search_stage_id)->pluck('id')->toArray();
        }
    ...
    {
        return Excel::download(new StudentsExport($this->getStudentQuery(), $this->getFiltersText()), 'students.xlsx');
    }
