...
class ChoicesOffline extends MaryChoicesOffline
{
    ...
    {
        return <<<'HTML'
                <div {{ $attributes->whereStartsWith('wire:key') }} x-data="{ focused: false, selection: @entangle($attributes->wire('model')) }">
                    <div
                        @click.outside = "clear()"
                        @keyup.esc = "clear()"
                        ...
                        x-data="{
                            ...
                            get selectedOptions() {
                                ...
                                return this.isSingle
                                    ...
                                    : (Array.isArray(this.selection) ? this.selection.map(i => this.options.filter(o => o.{{ $optionValue }} == i)[0]).filter(Boolean) : [])
                            },
                            get isAllSelected() {
                                return (Array.isArray(this.selection) && this.options.length == this.selection.length)
                            },
                            get isSelectionEmpty() {
                                return this.isSingle
                                    ? (this.selection === null || this.selection === undefined || this.selection === '')
                                    : (!this.selection || !Array.isArray(this.selection) || this.selection.length === 0)
                            },
                            selectAll() {
                            ...
                            isActive(id) {
                                return this.isSingle
                                    ? this.selection == id
                                    : (Array.isArray(this.selection) && this.selection.includes(id))
                            },
                                    ...
                                    <div x-ref="choicesOptions">
                                        @foreach($options as $option)
                                            <div
                                                ...
                                                wire:key="option-{{ $uuid }}-{{ data_get($option, $optionValue) }}"
