...
<head>
    ...
    <title>{{ $title }} - {{ siteSetting()->name ?? config('app.name', 'بوابة الأندلس') }}</title>
    ...
    <!-- Chart.js for High-Resolution Printing -->
...
<body class="p-4 md:p-8 min-h-screen">
    ...
    <div class="no-print max-w-5xl mx-auto mb-6 bg-white p-4 rounded-xl shadow-md border border-slate-200 flex flex-wrap items-center justify-between gap-4 sticky top-4 z-50">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-slate-800 text-base">{{ __('lang.print_preview') }}</h3>
                <p class="text-xs text-slate-500">{{ __('lang.print_ready') }}</p>
            </div>
        </div>
    ...
    <div class="print-container max-w-5xl mx-auto bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
        ...
        <header class="border-b-2 border-slate-800 pb-5 mb-6">
            <div class="flex items-center justify-between gap-4 mb-4">
                ...
                <!-- Report Title & Date -->
                <div class="text-left">
                    <span class="inline-block px-3 py-1 bg-slate-900 text-white font-bold text-xs rounded mb-1.5">{{ __('lang.official_report') }}</span>
                    <h2 class="text-lg font-black text-slate-900">{{ $title ?? __('lang.reports_mng') }}</h2>
            ...
            <div class="bg-slate-50 rounded-xl p-3.5 border border-slate-200 text-xs">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 {{ $selectedStudent ? 'md:grid-cols-5' : '' }} gap-2 text-slate-800">
                    <div class="p-2 bg-white rounded-lg border border-slate-200">
                        <span class="font-semibold text-slate-400 block text-[10px] mb-0.5">{{ __('lang.stage') }}:</span>
                        <span class="font-bold text-slate-900">{{ $selectedStage ?: __('lang.all_stages') }}</span>
                    </div>
                    <div class="p-2 bg-white rounded-lg border border-slate-200">
                        <span class="font-semibold text-slate-400 block text-[10px] mb-0.5">{{ __('lang.grade') }}:</span>
                        <span class="font-bold text-slate-900">{{ $selectedGrade ?: __('lang.all_grades') }}</span>
                    </div>
                    <div class="p-2 bg-white rounded-lg border border-slate-200">
                        <span class="font-semibold text-slate-400 block text-[10px] mb-0.5">{{ __('lang.section') }}:</span>
                        <span class="font-bold text-slate-900">{{ $selectedSection ?: __('lang.all_sections') }}</span>
                    </div>
                    <div class="p-2 bg-white rounded-lg border border-slate-200">
                        <span class="font-semibold text-slate-400 block text-[10px] mb-0.5">{{ __('lang.semester') }}:</span>
                        <span class="font-bold text-slate-900">{{ $selectedSemester ?: __('lang.all_semesters') }}</span>
                    </div>
                    @if($selectedStudent)
                        <div class="p-2 bg-white rounded-lg border border-indigo-200">
                            <span class="font-semibold text-indigo-500 block text-[10px] mb-0.5">{{ __('lang.student') }}:</span>
