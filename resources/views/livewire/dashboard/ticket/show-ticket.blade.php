    ...
    <x-header title="{{ $ticket->subject }}" subtitle="تذكرة رقم #{{ $ticket->id }}">
        <x-slot:actions>
            @can('edit_ticket')
                <x-dropdown>
                    <x-slot:trigger>
                        <x-button icon="o-chevron-down" class="btn-primary btn-sm" label="تغيير الحالة" />
                    </x-slot:trigger>
                    <x-menu-item title="مفتوحة" wire:click="changeStatus('open')" />
                    <x-menu-item title="قيد المراجعة" wire:click="changeStatus('in_progress')" />
                    <x-menu-item title="مغلقة" wire:click="changeStatus('closed')" />
                </x-dropdown>
            @endcan
            <x-button icon="o-arrow-right" class="btn-ghost btn-sm" link="{{ route('dashboard.tickets.index') }}"
                label="عودة للتذاكر" />
        </x-slot:actions>
    </x-header>
            ...
            <x-card title="المحادثة">
                <div class="space-y-6 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300">
                    @foreach ($ticket->replies as $reply)
                        ...
                        @endphp
                        <div class="chat {{ $isAdmin ? 'chat-start' : 'chat-end' }}">
                            ...
                            <div class="chat-bubble {{ $isAdmin ? 'chat-bubble-primary' : 'chat-bubble-neutral' }}">
                                {{ $reply->message }}
            ...
            <x-card title="تفاصيل التذكرة">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="font-bold">
                            {{ $ticket->assignedAdmin ? $ticket->assignedAdmin->name : 'لم يتم التعيين بعد' }}
                        </div>
                    </div>
                </div>
            </x-card>
