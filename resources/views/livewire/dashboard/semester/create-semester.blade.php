	...
	<x-modal wire:model="modalAdd" title="{{ __('lang.add') }} {{ __('lang.semester') }}" box-class="modal-box-600">
		<x-form wire:submit="saveAdd">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
				...
				<x-choices-offline label="{{ __('lang.grade') }}" wire:model="grade_id" :options="$all_grades" option-value="id" option-label="name" single searchable placeholder="{{ __('lang.select') }}..." :disabled="!$stage_id"/>
			</div>
			<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
				<x-input type="number" label="{{ __('lang.academic_year_from') }}" wire:model="academic_year_from" placeholder="2024" icon="o-calendar"/>
				<x-input type="number" label="{{ __('lang.academic_year_to') }}" wire:model="academic_year_to" placeholder="2025" icon="o-calendar"/>
			</div>
			<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
