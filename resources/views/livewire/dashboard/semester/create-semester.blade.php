<div>
    <x-button icon="o-plus" class="btn-primary btn-sm mt-2 md:mt-0" label="{{ __('lang.add') }}"
        @click="$wire.modalAdd = true" wire:click="resetData" />
    <x-modal wire:model="modalAdd" title="{{ __('lang.add') }} {{ __('lang.semester') }}" box-class="modal-box-600">
        <x-form wire:submit="saveAdd">
            ...
            <x-checkbox label="هل تريد إنشاء الأسابيع الخاصة بهذا الفصل بشكل تلقائي؟"
                ...
                hint="سيتم إنشاء الأسابيع بناءً على تاريخ بدء ونهاية الفصل الدراسي" />
            <x-slot:actions>
                <x-button label="{{ __('lang.cancel') }}" @click="$wire.modalAdd = false" />
                <x-button label="{{ __('lang.save') }}" class="btn btn-primary" wire:loading.attr="disabled"
                    type="submit" spinner="saveAdd" />
            </x-slot:actions>
        </x-form>
    </x-modal>
</div>
