    ...
    <x-card title="{{ __('lang.students') }}" shadow class="mb-3">
        <x-slot:menu>
            <div class="flex flex-wrap items-center justify-end gap-2">
                @can('edit_student')
                    <x-button icon="o-check-badge" class="btn-primary btn-sm" wire:click="confirmActivatePending"
                        tooltipBottom="تفعيل كل الطلاب قيد الانتظار وإرسال إيميلات التفعيل لهم" spinner="confirmActivatePending"
                        label="تفعيل المعلقين ({{ $global_pending_count }})" />
                @endcan
                <x-button icon="o-document-arrow-down" class="btn-success btn-sm" wire:click="exportExcel"
                    tooltip="تصدير Excel" spinner="exportExcel" />
                <x-button icon="o-document-arrow-down" class="btn-error btn-sm" wire:click="exportPdf" tooltip="تصدير PDF"
                    spinner="exportPdf" />
                @can('create_student')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-5">
            ...
            <x-stat title="الطلاب المفعلين" value="{{ $active_students }}" icon="o-check-circle"
                class="bg-success/20 text-success" />
            <x-stat title="قيد الانتظار" value="{{ $pending_students }}" icon="o-clock"
                class="bg-warning/20 text-warning" />
            <x-stat title="الطلاب غير المفعلين" value="{{ $inactive_students }}" icon="o-x-circle"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
            ...
            <x-choices-offline label="{{ __('lang.grade') }}" wire:model.live="search_grade_id" :options="$all_grades"
                option-sub-label="full_path_name" option-value="id" option-label="name" single clearable searchable
                placeholder="{{ __('lang.search') }}"
                wire:key="filter-student-grade-select-{{ $search_stage_id ?? 'all' }}" />
            <x-choices-offline label="{{ __('lang.section') }}" wire:model.live="search_section_id" :options="$all_sections"
                option-sub-label="full_path_name" option-value="id" option-label="name" single clearable searchable
                placeholder="{{ __('lang.search') }}"
                wire:key="filter-student-section-select-{{ $search_stage_id ?? 'all' }}-{{ $search_grade_id ?? 'all' }}" />
            <x-choices-offline label="{{ __('lang.semester') }}" wire:model.live="search_semester_id" :options="$all_semesters"
                option-value="id" option-label="name" option-sub-label="full_path_name" single clearable searchable
                placeholder="{{ __('lang.search') }}"
                wire:key="filter-student-semester-select-{{ $search_stage_id ?? 'all' }}-{{ $search_grade_id ?? 'all' }}" />
            <x-choices-offline label="{{ __('lang.week') }}" wire:model.live="search_week_id" :options="$all_weeks"
                option-value="id" option-label="name" option-sub-label="full_path_name" single clearable searchable
                placeholder="{{ __('lang.search') }}"
                wire:key="filter-student-week-select-{{ $search_stage_id ?? 'all' }}-{{ $search_grade_id ?? 'all' }}-{{ $search_semester_id ?? 'all' }}" />
                            ...
                            <tr class="bg-base-200">
                                        ...
                                        @if ($student->getFirstMediaUrl('image'))
                                            <div class="avatar">
                                                <div class="w-8 rounded-full">
                                                    <img src="{{ $student->getFirstMediaUrl('image') }}"
                                                        alt="{{ $student->name }}" />
                                                </div>
                                            </div>
                                        @endif
                                ...
                                <td>
                                    <div class="flex gap-2 justify-center">
                                        @can('show_student')
                                            <x-button icon="o-chart-bar" class="btn-sm btn-ghost text-info"
                                        ...
                                        @can('edit_student')
                                            ...
                                            <livewire:dashboard.student.update-student :student="$student"
    ...
    <x-modal wire:model="showActivatePendingModal" title="تأكيد تفعيل الطلاب قيد الانتظار" box-class="modal-box-600">
        <div class="space-y-4 text-sm">
            <div class="p-4 bg-primary/10 text-primary border border-primary/20 rounded-xl flex items-center gap-3.5">
                ...
                <div>
                    <div class="font-bold text-base text-base-content">إجمالي الطلاب قيد الانتظار: <span
                            class="text-primary font-black">{{ $pendingStudentsCount }}</span> طالب</div>
                    <div class="text-xs text-base-content/70 mt-0.5">سيتم تحويل حالتهم إلى "مفعل" وإرسال الإيميلات
                        بالخلفية عبر الـ Queue Job.</div>
                </div>
            </div>
            ...
            <div class="p-3.5 bg-base-200/60 rounded-xl border border-base-300 space-y-2 text-xs text-base-content/80">
                ...
                <p
                    class="italic text-base-content/70 bg-base-100 p-2.5 rounded-lg border border-base-300 leading-relaxed">
                    "هلا والله [اسم الطالب]، أبشرك! تم تفعيل حسابك في المنصة بنجاح 🎉. يمديك الحين تسجل دخول وتستفيد من
                    كامل خدمات المنصة، وتتابع اختباراتك وتدريباتك بكل سهولة."
                </p>
            </div>
        ...
        <x-slot:actions>
            ...
            <x-button label="أبشر، فعّل وأرسل الإيميلات" class="btn-primary" wire:click="activateAllPendingStudents"
                spinner="activateAllPendingStudents" icon="o-paper-airplane" />
        </x-slot:actions>
    </x-modal>
</div>
