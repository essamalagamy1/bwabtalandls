...
<div>
	...
	<x-modal wire:model="modalUpdate" title="{{__('lang.update')}}" box-class="modal-box-600">
		<x-form wire:submit="saveUpdate">
			...
			<x-choices-offline searchable single clearable label="{{__('lang.university')}}" wire:model="university_id" :options="$universities" placeholder="{{__('lang.select')}}" option-value="id" option-label="name"/>
