<div>
    <x-header title="{{ __('lang.my_exams') ?? 'امتحاناتي' }}"
        subtitle="{{ __('lang.all_exams_for_your_grade') ?? 'جميع الامتحانات المخصصة لصفك الدراسي' }}" separator />

    <div class="mb-6 bg-base-100 p-4 rounded-xl shadow-sm border border-base-200">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
            @if (isset($stages) && count($stages) > 0)
                <x-select icon="o-academic-cap" :options="$stages" option-value="id" option-label="name"
                    placeholder="جميع المراحل" wire:model.live="selectedStage" />
            @endif

            @if (isset($filteredGrades) && count($filteredGrades) > 0)
                <x-select icon="o-book-open" :options="$filteredGrades" option-value="id" option-label="name"
                    placeholder="جميع الصفوف" wire:model.live="selectedGrade" />
            @endif

            @if (isset($semesters) && count($semesters) > 0)
                <x-select icon="o-calendar" :options="$semesters" option-value="id" option-label="name"
                    placeholder="{{ __('lang.all_semesters') ?? 'الفصول المتاحة' }}" wire:model.live="selectedSemester" />
            @endif

            @if (!empty($weeks) && count($weeks) > 0)
                <x-select icon="o-calendar-days" :options="$weeks" option-value="id" option-label="title"
                    placeholder="{{ __('lang.all_weeks') ?? 'جميع الأسابيع' }}" wire:model.live="selectedWeek" />
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($exams as $exam)
            ...
            <x-card title="{{ $exam->title }}" class="shadow-xl">
                ...
                <x-slot:actions>
                    @if ($attempt)
                        <x-button label="{{ __('lang.view_result') }}" class="btn-primary w-full"
                            link="{{ route('student.exams.result', $exam->id) }}" icon="o-eye" />
                    @else
                        <x-button label="{{ __('lang.enter_exam') }}" class="btn-primary w-full"
                            link="{{ route('student.exams.take', $exam->id) }}" icon="o-play" />
                    @endif
                </x-slot:actions>
            </x-card>
        @empty
